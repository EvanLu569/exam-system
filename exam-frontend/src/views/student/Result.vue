<template>
  <div class="page result-page">
    ...
    <template v-else-if="record">
      ...
      <el-card shadow="never" class="detail-card">
        <div class="detail-head">
          ...
          <span class="detail-stat">
            答对 <b class="ok">{{ record.correctCount }}</b> / {{ record.totalCount }} 题
          </span>
        </div>
        ...
        <div v-for="(d, i) in record.questions" :key="d.questionId" class="detail-item">
          ...
          <div class="d-answer">
            <div>
              你的答案：<span :class="d.isCorrect ? 'ok' : 'bad'">
                {{ answerText(d.userAnswer, d.type) || '未作答' }}
              </span>
            </div>
            <div v-if="!d.isCorrect">
              正确答案：<span class="ok">{{ answerText(d.correctAnswer, d.type) }}</span>
            </div>
          </div>
        </div>
      </el-card>
      ...
      <div class="actions">
        <el-button type="primary" @click="$router.replace('/exams')">返回考试列表</el-button>
        <el-button @click="$router.push('/records')">查看全部记录</el-button>
      </div>
    </template>
  </div>
</template>
